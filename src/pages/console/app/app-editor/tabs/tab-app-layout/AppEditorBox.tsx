...
import {EditorBoxPosition} from "../../../../../../hype/classes/constant";
import GenerateAppComponent from "../../../../../../hype/components/GenerateAppElement";
import {AppModeType} from "../../../../../../libs/util";


export const AppEditorBox = (props: { boxId: string, path: Array<number> }) => {
    const {boxId, path} = props;
    const store = useBoundStore(state => state.appEditor)
    const boxData = useBoundStore(state => {
        return state.appEditor.layoutItemList.find(d => d.id == boxId)
    })
                        ...
                        boxData.type === 'container' ? (<Container id={`e_${boxId}`} style={{
...

                            {
                                boxData.children != null && boxData.children.map((boxId, k) => {
                                    return <AppEditorBox path={[...path, k]} boxId={boxId} key={k}/>
                                })
                            }
                        </Container>) : null
                    }
